...
export function downloadExamQuestionsPDF(questions: Question[]): void {
  ...
  questions.forEach((q, idx) => {
    ...
    if (q.type === 'pgk_kategori') typeLabel = 'Pilihan Ganda Kompleks Kategori';
    ...
    if (q.imageSvg) {
      ...
      doc.text('[Lihat diagram ilustrasi visual pada tampilan aplikasi ujian]', 16, y);
      y += 5;
      doc.setFont('helvetica', 'normal');
      doc.setFontSize(9.5);
      doc.setTextColor(0, 0, 0);
    ...
    // Pernyataan Kategori (Benar / Salah, dll)
    if (q.statements && q.statements.length > 0) {
      const posLabel = q.categoryResponseType === 'setuju_tidak_setuju' ? 'Setuju' : q.categoryResponseType === 'sesuai_tidak_sesuai' ? 'Sesuai' : 'Benar';
      const negLabel = q.categoryResponseType === 'setuju_tidak_setuju' ? 'Tdk Setuju' : q.categoryResponseType === 'sesuai_tidak_sesuai' ? 'Tdk Sesuai' : 'Salah';
  ...
  doc.save(`Naskah_Soal_IPAS_Kelas_VI_${CONFIG.SEKOLAH.replace(/[^a-zA-Z0-9]/g, '_')}.pdf`);
}
