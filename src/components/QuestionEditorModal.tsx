...
export const QuestionEditorModal: React.FC<QuestionEditorModalProps> = ({
  ...
  const [isianAnswer, setIsianAnswer] = useState<string>(
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const parsedAcceptable = acceptableAnswersText
      .split(',')
      .map((s) => s.trim())
      .filter(Boolean);
    ...
    const updated: Question = {
      ...
      correctAnswer:
        question.type === 'isian'
          ...
          ? correctAnswer
          : undefined,
      acceptableAnswers: question.type === 'isian' ? parsedAcceptable : undefined,
    };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs animate-in fade-in duration-200">
      <div className="bg-white rounded-2xl max-w-2xl w-full p-6 shadow-2xl border border-slate-200 relative max-h-[90vh] flex flex-col">
        ...
        <form onSubmit={handleSubmit} className="overflow-y-auto py-4 space-y-4 pr-1 text-xs sm:text-sm">
          ...
          {/* Opsi untuk Isian Singkat */}
          {question.type === 'isian' && (
            <div className="space-y-3 p-3.5 rounded-xl border border-purple-200 bg-purple-50/50">
              <div>
                <label className="block text-xs font-bold text-slate-700 mb-1">
                  Kunci Jawaban Utama (Kata / Istilah Ilmiah)
                </label>
                <input
                  type="text"
                  value={isianAnswer}
                  onChange={(e) => setIsianAnswer(e.target.value)}
                  placeholder="Contoh: Alveolus"
                  className="w-full px-3 py-2 rounded-lg border border-slate-300 text-xs font-bold text-slate-800 bg-white focus:ring-2 focus:ring-purple-500 focus:outline-none"
                  required
                />
              </div>
              <div>
                <label className="block text-xs font-bold text-slate-700 mb-1">
                  Variasi Jawaban Diterima (Pisahkan dengan tanda koma)
                </label>
                <input
                  type="text"
                  value={acceptableAnswersText}
                  onChange={(e) => setAcceptableAnswersText(e.target.value)}
                  placeholder="Contoh: alveolus, alveoli, gelembung paru-paru"
                  className="w-full px-3 py-2 rounded-lg border border-slate-300 text-xs font-medium text-slate-800 bg-white focus:ring-2 focus:ring-purple-500 focus:outline-none"
                />
                <p className="text-[11px] text-slate-500 mt-1">
                  Siswa yang mengetikkan salah satu dari variasi di atas (tidak peka huruf besar/kecil) akan tetap dinilai benar.
                </p>
              </div>
            </div>
          )}
